...
from models.wavelet_transform_exploration import MultiscaleWaveletTransformer2D
...
def train_step_ahead_residual(model, mean_model, train_loader, optimizer, scheduler, config, device, grid, test_loader=None, eval_step=100,save_step=1000, use_tqdm=True, writer=None, model_name='fno2d', start_ep=0):
    """Train on residuals (u_{t+1} - u_{t+1_pred}, model(u_t))."""
   

    lploss = LpLoss(size_average=True)
    epochs = 2*config['train']['epochs']
    grid = grid.to(device).unsqueeze(0)
    if start_ep >= epochs:
        print(f'start_ep ({start_ep}) >= epochs ({epochs}); skipping training loop.')
        return
...
def train_3d(args, config):
    ...
    if model_name == 'hfs':
        ...
        model = MultiscaleWaveletTransformer2D(
            ...
            rescaling=model_cfg.get('rescaling', False),
        ).to(device)

        model.enable_mean_training()
        # create optimizer and learning rate scheduler
        optimizer = Adam(model.trainable_param_groups(), betas=(0.9, 0.999),
    ...
    train_step_ahead_residual(
                        ...
                        test_loader=test_loader,
                        writer=writer,
                        model_name='residual_'+model_name,
                        start_ep=config['train']['epochs']+1)
