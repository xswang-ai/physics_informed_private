...
def time_dependent_eval(model, sequences, device):
    """Run time-dependent evaluation on full sequences.

    Assumes the model was trained with normalized time encoding: t_idx / (T-1)."""
    lploss = LpLoss(size_average=True)
    model.eval()
    S = sequences.shape[1]
    T = sequences.shape[-1]
    ...
    with torch.no_grad():
        for (seq,) in loader:
            seq = seq.to(device)  # (1, S, S, T)
            B, H, W, T = seq.shape
            x_init = seq[..., 0]  # initial condition
            preds = []
            max_t_idx = T - 1
            for t_idx in range(1, T):
                # match training: normalized time t_idx / max_t_idx
                t_norm = float(t_idx) / float(max_t_idx)
                time_idx = torch.full((B, H, W, 1), t_norm, device=seq.device, dtype=seq.dtype)
                x_in = torch.cat((x_init.unsqueeze(-1), grid.expand(x_init.shape[0], -1, -1, -1), time_idx), dim=-1)
                pred = model(x_in)
                if pred.dim() == 5:
                    pred = pred.squeeze(-2)
                if pred.dim() == 4:
...
def main():
    ...
    l2, example = time_dependent_eval(model, sequences, device)
